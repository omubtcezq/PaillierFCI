...
\subsection{Notation}
Throughout this work, the following notation is used. Plain characters, $a$, denote scalars, underlined lowercase characters, $\vec{b}$, denote vectors and uppercase bold characters, $\mat{C}$, denote matrices. $\mat{C}^{-1}$ denotes the matrix inverse and $\tr(\mat{C})$ the matrix trace. Encryption with a public key $\mathsf{pk}$ is denoted $\mathcal{E}_{\mathsf{pk}}(\cdot)$ and decryption with a private key $\mathsf{sk}$ is denotes $\mathcal{D}_{\mathsf{sk}}(\cdot)$, while encoding and decoding with parameters $M$ and $\phi$ are denoted $\mathsf{E}_{m,\phi}(\cdot)$ and $\mathsf{E}^{-1}_{m,\phi}(\cdot)$, respectively.
...
Simulataneously, we consider the desired security of the fusion process. The fusing cloud and sensors are treated as \textit{honest-but-curious}, that is, they follow protocols correctly but may use learned information for malicious gain, and a trusted third party exists that can query the cloud at any timestep $k$ to obtain the fusion results in \eqref{eqn:fus_ests_and_covs}. To guarantee that no new information about estimates, \eqref{eqn:ests_and_covs}, and fusions, \eqref{eqn:fus_ests_and_covs}, can be learned by eavesdroppers, the cloud and the sensors, we aim for cryptographic \textit{Indistinguishability under the Chosen Plaintext Attack} (IND-CPA) \cite{katzIntroductionModernCryptography2008} for all transimitted and processed information. This is inline with common security goals in the field and suitable for homomorphic encryption.

% TODO should say third party can't learn individual inputs beyond the fused result too
...
\begin{equation}
    ...
    \end{split}
\end{equation}
combine information from individual estimators $i$ and are computable homomorphically given suitable encryptions. Since these sums can be computed from the final fusion results \eqref{eqn:ci_cov_fusion} and \eqref{eqn:ci_est_fusion}, they contain no additional information on individual estimator contributions than the results themselves and can be decrypted by the key-holding third party without affecting leakage. The remaining inversions and multiplications in \eqref{eqn:fci_cov_rearrange} and \eqref{eqn:fci_est_rearrange} can then be computed by the third party to obtain the final results.



% TODO say that this additionally supports sequential fusion on the cloud as the scaling component is only additive
...
% TODO point out that the third party also doesn't learn individual contributions (as should be updated in the problem statement as well)
