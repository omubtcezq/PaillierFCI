...
%\usepackage{flushend} % Sometime breaks order of references
% Algorithms
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}
        % Bounding box
        \draw [gray] (1,0) rectangle (8.5,7);
        % Estimators
        \node [above] at (4.75,6.25) {Estimators};
        \fill [pyplotblue!70] (7,5.5) ellipse (0.5 and 0.5);
        \fill [pyplotblue!70] (2.5,5.5) ellipse (0.5 and 0.5);
        ...
        % Fusion
        \node [right] at (5.25,1.5) {$\hat{\vec{x}}_{k,\mathsf{fus}},\mat{P}_{k,\mathsf{fus}}$};
        % Cloud
        \node [right] at (5.5,3.5) {Cloud};
        \fill [pyplotorange!70] (4.75,3.5) ellipse (0.5 and 0.5);
        % Third-party
        \node [below] at (4.75,0.75) {Trusted Third-party};
        \fill [pyplotgreen!70] (4.75,1.5) ellipse (0.5 and 0.5);
        % Estimator arrows
        \draw [-latex]  plot coordinates {(3,5)  (4.25,4)};
        \draw [-latex]  plot coordinates {(6.5,5)  (5.25,4)};
        % Third-party arrows
        \draw [latex-latex]  plot coordinates {(4.75,2.9)  (4.75,2.1)};
    \end{tikzpicture}
    \caption{The participants, inputs and output of the considered fusion scenario.}
    \label{fig:layout}
\end{figure}
